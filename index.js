...
app.get('/view/city/:id', function(request, response) {
	...
	var viewPersonsByCity = queries.allPersons + " WHERE city.id = " + mysql.escape(request.params.ID) + " ORDER BY id; ";
	var viewIncidentsByCity = queries.allIncidents + " WHERE city.id = " + mysql.escape(request.params.ID) + " ORDER BY id; ";
