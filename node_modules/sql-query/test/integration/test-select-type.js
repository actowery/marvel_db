...
assert.equal(
	common.Select().from('table1').fun('myfun', [ 'col1', 'col2'], 'name').build(),
	"SELECT MYFUN(`col1`, `col2`) AS `name` FROM `table1`"
);

assert.equal(
	common.Select().from('table1').fun('myfun', [ 'col1', common.Text('col2') ], 'name').build(),
	"SELECT MYFUN(`col1`, 'col2') AS `name` FROM `table1`"
);
