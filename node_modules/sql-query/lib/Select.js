...
function SelectQuery(Dialect, opts) {
	...
	var get_table_name = function (table) {
	...
	var aggregate_fun = function (fun) {
		return function () {
			...
			var name  = (column.length > 1 && typeof column[column.length - 1] == "string" ? column.pop() : null);

			if (column.length && Array.isArray(column[0])) {
				column = column[0].concat(column.slice(1));
			}

			return this.fun(fun, column.length && column[0] ? column : '*', name);
		};
	};
	var proto = {
		...
		as: function (name) {
			...
			if (sql.from[idx].select.length) {
				...
				sql.from[idx].select[sql.from[idx].select.length - 1].a = name || null;
			}

			return this;
		},
		fun: function (fun, column, name) {
		...
		whereExists: function (table, table_link, link, conditions) {
			sql.where.push({
				...
				w: conditions,
				e: { t: table, tl: get_table_name(table_link), l: link }
			});
		...
		order: function (column, dir) {
			if (Array.isArray(dir)) {
				...
				sql.order.push({
					c : Array.isArray(column) ? [ get_table_name(column[0]), column[1] ] : column,
					d : (dir == "Z" ? "DESC" : "ASC")
				});
			}
			return this;
