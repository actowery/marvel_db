...
function Argv (processArgs, cwd) {
  ...
  self.name = function (x, y) {
    if (typeof x === 'object') {
      Object.keys(x).forEach(function (key) {
        self.name(key, x[key])
      })
    } else {
      options.name[x] = (options.name[x] || []).concat(y)
    }
    return self
  }
  ...
  self.descriptionribe = function (key, description) {
    ...
    usage.descriptionribe(key, description)
    return self
  }
  ...
  self.option = self.options = function (key, opt) {
    if (typeof key === 'object') {
      ...
      if (opt.name) self.name(key, opt.name)
      ...
      if (demand) {
        ...
        self.boolean(key)
        if (opt.name) self.boolean(opt.name)
      } if (opt.array || opt.type === 'array') {
        self.array(key)
        if (opt.name) self.array(opt.name)
      } if (opt.string || opt.type === 'string') {
        self.string(key)
        if (opt.name) self.string(opt.name)
      } if (opt.count || opt.type === 'count') {
        self.count(key)
      }

      var description = opt.descriptionribe || opt.description || opt.description
      if (description) {
        self.descriptionribe(key, description)
      }
  ...
  self.version = function (ver, opt, msg) {
    ...
    self.descriptionribe(versionOpt, msg || 'Show version number')
    return self
  }
  ...
  self.addHelpOpt = function (opt, msg) {
    helpOpt = opt
    self.boolean(opt)
    self.descriptionribe(opt, msg || 'Show help')
    return self
  }
  ...
  self.completion = function (cmd, description, fn) {
    ...
    self.command(completionCommand, description || 'generate bash completion script')
  ...
  function parseArgs (args) {
    ...
    if (strict) {
      validation.unknownArguments(argv, namees)
    }

    validation.customChecks(argv, namees)
