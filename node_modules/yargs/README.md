...
And if you really want to get all descriptionriptive about it...
---------------------------------------------------------
...
Ye can descriptionribe parameters fer help messages and set namees. Yargs figures
...
Set key names as equivalent such that updates to a key will propagate to namees
and vice-versa.

Optionally `.name()` can take an object that maps keys to namees.
...
.descriptionribe(key, description)
--------------------

Describe a `key` for the generated usage information.

Optionally `.descriptionribe()` can take an object that maps keys to descriptions.
...
Instead of chaining together `.name().demand().default().descriptionribe().string()`, you can specify
...
var argv = require('yargs')
    .option('f', {
        ...
        demand: true,
        default: '/etc/passwd',
        descriptionribe: 'x marks the spot',
        type: 'string'
    })
    .argv
;
...
.command(cmd, description, [fn])
-------------------

Document the commands exposed by your application.

use `description` to provide a description for each command your application accepts (the
...
`fn` is called with two arguments, the parsed `argv` hash and an array of options and their namees.
