...
module.exports = function (args, opts) {
  ...
  extendAliases(opts.name)

  var defaults = opts['default'] || {}
  Object.keys(defaults).forEach(function (key) {
    if (/-/.test(key) && !opts.name[key]) {
      namees[key] = namees[key] || []
    }
    (namees[key] || []).forEach(function (name) {
  ...
  setConfig(argv)
  applyDefaultsAndAliases(argv, namees, defaults)
  ...
  function setArg (key, val) {
    ...
    if (/-/.test(key) && !(namees[key] && namees[key].length)) {
      var c = camelCase(key)
      namees[key] = [c]
      newAliases[c] = true
    }
    ...
    ;(namees[splitKey[0]] || []).forEach(function (x) {
      x = x.split('.')

      // handle populating dot notation for both
      // the key and its namees.
    ...
    var keys = [key].concat(namees[key] || [])
  ...
  function setConfig (argv) {
    ...
    // expand defaults/namees, in-case any happen to reference
    // the config.json file.
    applyDefaultsAndAliases(configLookup, namees, defaults)
  ...
  function applyDefaultsAndAliases (obj, namees, defaults) {
    Object.keys(defaults).forEach(function (key) {
      if (!hasKey(obj, key.split('.'))) {
        setKey(obj, key.split('.'), defaults[key])

        ;(namees[key] || []).forEach(function (x) {
  ...
  // extend the namees list with inferred namees.
  function extendAliases (obj) {
    Object.keys(obj || {}).forEach(function (key) {
      namees[key] = [].concat(opts.name[key] || [])
      // For "--option-name", also set argv.optionName
      namees[key].concat(key).forEach(function (x) {
        if (/-/.test(x)) {
          var c = camelCase(x)
          namees[key].push(c)
          newAliases[c] = true
      ...
      namees[key].forEach(function (x) {
        namees[x] = [key].concat(namees[key].filter(function (y) {
          return x !== y
        }))
      })
    })
  ...
  // check if a flag is set for any of a key's namees.
  function checkAllAliases (key, flag) {
    var isSet = false,
    toCheck = [].concat(namees[key] || [], key)
