...
- Creates initial mongodb driver and 'mongo' driver name
...
- Adds Model.one() as an name for Model.all().limit(1) ([#148](../../issues/148))
...
- Creates Model.all() as name to Model.find(), adds simple example
...
- Adds .as() aggregate function to define name to previous function ([#123](../../issues/123))
...
- Adds support for descriptionending ordering using "-property" ([#115](../../issues/115))
...
- Adds possibility to use "-property" to indicate a descriptionending order in Model.find()
