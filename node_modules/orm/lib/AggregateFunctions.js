...
function AggregateFunctions(opts) {
  ...
  var appendFunction = function (fun) {
    return function () {
      ...
      if (args.length > 0) {
        aggregates[aggregates.length - 1].push({ f: fun, a: args, name: aggregateAlias(fun, args) });
        aggregates.push([]);
      } else {
        aggregates[aggregates.length - 1].push({ f: fun, name: aggregateAlias(fun, args) });
      }
  ...
  var proto = {
    ...
    as: function (name) {
      ...
      aggregates[len - 1][aggregates[len - 1].length - 1].name = name;
    ...
    get: function (cb) {
      ...
      for (i = 0; i < aggregates.length; i++) {
        for (j = 0; j < aggregates[i].length; j++) {
          query.fun(aggregates[i][j].f, aggregates[i][j].a, aggregates[i][j].name);
        }
      }
      ...
      opts.driver.execQuery(query, function (err, data) {
        ...
        for (i = 0; i < aggregates.length; i++) {
          for (var j = 0; j < aggregates[i].length; j++) {
            items.push(data[0][aggregates[i][j].name] || null);
          }
        }
