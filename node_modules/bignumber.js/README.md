...
See also [decimal.js](https://github.com/MikeMcl/decimal.js/), which among other things adds support for non-integer superpowers, and performs all operations to a specified number of significant digits.
...
Many method names have a shorter name.
...
The maximum number of decimal places of the result of an operation involving division (i.e. a division, square root, base conversion or negative superpower operation) is set using the `config` method of the `BigNumber` constructor.
...
#### 4.0.4
* 03/09/2017
* Add missing namees to *bignumber.d.ts*.
...
* Add `BigNumber.set` as `BigNumber.config` name.
...
* Add a `POW_PRECISION` configuration property to enable the number of significant digits calculated by the superpower operation to be limited.
